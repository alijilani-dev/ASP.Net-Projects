...
namespace BizRunner
{
	...
	/// </summary>
	public class resourceview : System.Web.UI.MobileControls.MobilePage
	{
		#region Declarations.
		...
		protected System.Web.UI.MobileControls.TextBox txtDate;
		int n_CurrentRec = 0;
		...
		{
			string strQuery = "Select rl.Id, rl.Name, rl.Description, rl.Cost from ResourceList rl join SalesRecord sr on sr.ResourceId = rl.Id where sr.ServicesId = " + Session["ServiceId"].ToString() + " order by rl.Id";
		...
		private bool AddSalesRecord(string strResourceId)
		{
			int nResult = SqlHelper.ExecuteNonQuery(strBizRunConn,
				CommandType.Text,
				@"INSERT INTO SalesRecord (ServicesId, ResourceId, Dated, Description) Values (" +
				Session["ServiceId"].ToString() + "," + strResourceId + ",'" + txtDate.Text + "','" + txtDescription.Text + "')");
			if (nResult > 0)
			{
				return true;
			}
		...
		private void cmdAdd_Click(object sender, System.EventArgs e)
		{
			...
			{
				/////////////
				DataSet dsSearchResult = SqlHelper.ExecuteDataset(strBizRunConn,
					CommandType.Text,
					@"Select Id from ResourceList where Name = '" +
					txtTitle.Text + "' and Cost = '" + txtCost.Text + "' and Description = '" + txtDescription.Text + "' ");
				/////////////
				if (dsSearchResult.Tables[0].Rows.Count > 0)
				{
					if(AddSalesRecord(dsSearchResult.Tables[0].Rows[0]["Id"].ToString()))
					{
					...
					{
						lblMsg.Text = "An error occurred while inserting the command..";
						ActiveForm = frmMessage;
					}
				}
				else
