...
namespace BizRunner
{
	...
	public class customerview : System.Web.UI.MobileControls.MobilePage
	{
		...
		{    
			...
			this.cmdManageServices.Click += new System.EventHandler(this.cmdManageServices_Click);
		...
		private DataSet GetCustomerData()
		{
			...
			string strQuery = @"Select DISTINCT cl.Id, cl.CompanyName, cl.ContactName, cl.Address, cl.Mobile, cl.Telephone, cl.Email 
								From CustomerList cl join userlist ul on cl.RepresentativeId = ul.Id
								where cl.RepresentativeId = " + Session["UserId"].ToString() + " order by cl.Id";
		...
		private void CustomersList_ItemCommand(object sender, System.Web.UI.MobileControls.ListCommandEventArgs e)
		{
			...
			Session["CustomerIndex"] = e.ListItem.Index;
			Session["CustomerID"] = e.ListItem.Value;
		...
		{
			int nIndex = int.Parse(Session["CustomerIndex"].ToString());
			int n_CurrentRec = 0;
			if (nIndex>0)
			{
				nIndex--;
				...
				Session["CustomerID"] = n_CurrentRec;
				Session["CustomerIndex"] = nIndex;
			}
			else
			{
		...
		private void cmdAdd_Click(object sender, System.EventArgs e)
		{
			int nResult = SqlHelper.ExecuteNonQuery(strBizRunConn,
				CommandType.Text,
				@"INSERT INTO CustomerList(RepresentativeId,CompanyName, ContactName, Address, Telephone, Mobile, Email) Values (" +
				Session["UserId"].ToString() + ",'" +txtCompanyName.Text + "','" + txtContactName.Text + "','" + txtAddress.Text + "','" +
		...
		private void cmdManageServices_Click(object sender, System.EventArgs e)
		{
