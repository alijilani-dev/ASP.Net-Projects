...
namespace BizRunner
{
	...
	public class RolesAssignment : System.Web.UI.MobileControls.MobilePage
	{
		...
		private void GetUsers()
		{
			...
			string strQuery = "select Id, LoginName from UserList order by Id";
			...
			// Search for item with current user value;
			for( int i = 0; i < slUsers.Items.Count; i++ )
			{
				slUsers.SelectedIndex = i;
				if (slUsers.Selection.Value.ToString() == nSelectedUser.ToString())
					break;
			}
			// Select the item if matches current user value;
