...
namespace BizRunner
{
	...
	public class taskview : System.Web.UI.MobileControls.MobilePage
	{
		...
		{
			string strQuery = "select sr.Id, sr.Title, sr.CustomerId, sr.RepresentativeId, sr.ServicesType, sr.Status, sr.StartDate, sr.EndDate, sr.Description from ServicesRecord sr order by Id";
		...
		private void ServicesList_ItemCommand(object sender, System.Web.UI.MobileControls.ListCommandEventArgs e)
		{
			...
			Session["CurrentIndex"] = e.ListItem.Index;
		...
		{
			int nIndex = int.Parse(Session["CurrentIndex"].ToString());
			int n_CurrentRec = 0;
			if (nIndex>0)
			{
				nIndex--;
