...
namespace Evernet.MoneyExchange.Web.DropDownLists {
	...
	/// <summary>
	/// This class derives from System.Web.UI.WebControls.DropDownList class and was built to display the 'AgentAccountType' table content.
	/// </summary>
#if OLYMARS_ATTRIBUTE || OLYMARS_DEBUG
	[Evernet.MoneyExchange.DataClasses.OlymarsInformation(DeveloperName="Evernet Technologies LLC", GeneratedOn="2004/11/30 19:47:37", SqlObjectDependancyName="AgentAccountType", SqlObjectDependancyRevision=192)]
#endif
	[System.Drawing.ToolboxBitmap(typeof(WebDropDownList_AgentAccountType), "WebDropDownList.bmp")]
	public class WebDropDownList_AgentAccountType : System.Web.UI.WebControls.DropDownList {

		private Params.spS_AgentAccountType_Display param;
		private Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType oAbstract_AgentAccountType;
		...
		/// </summary>
		public Params.spS_AgentAccountType_Display SP_Parameter {
		...
		/// <summary>
		/// Returns the 'AgentAccountType' abstract class so that you can access any table
		...
		/// </summary>
		public Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType Abstract_AgentAccountType {

			get {

				return(this.oAbstract_AgentAccountType);
			}
		}
		...
		public override void Dispose() {
			...
			this.oAbstract_AgentAccountType = null;
		...
		/// <summary>
		/// Create a new instance of the Evernet.MoneyExchange.Web.DropDownLists.WebDropDownList_AgentAccountType class.
		/// </summary>
		public WebDropDownList_AgentAccountType() : base() {
		...
		public void Initialize(string connectionString) {
			...
			if (olymarsDebugCheck == null || (string)olymarsDebugCheck == "True") {
				...
				if (DebugConnectionString.Length != 0) {
					...
					sqlCommand.CommandText = "Select sysobjects.schema_ver from sysobjects where sysobjects.name = 'AgentAccountType'";
					...
					if (CurrentRevision != OriginalRevision) {
						...
						throw new System.InvalidOperationException(System.String.Format("OLYMARS: This code is not in sync anymore with [{0}]. It was generated when [{0}] version was: {2}. Current [{0}] version is: {1}{3}{3}You can either regenerate the code for this class so that it will be based on the new version or edit the configuration file of the class caller application and paste the following code:{3}{3}<?xml version=\"1.0\" encoding=\"utf-8\" ?>{3}<configuration>{3}\t<appSettings>{3}\t\t<add key=\"OlymarsDebugCheck\" value=\"False\" />{3}\t</appSettings>{3}</configuration>{3}{3}You will need to reload the caller application if it is a Windows Forms based application.", "AgentAccountType", CurrentRevision, OriginalRevision, System.Environment.NewLine));
					}
				}
			}
#endif
		...
		public void RefreshData(System.Data.SqlTypes.SqlString PK_Value, int startRecord, int maxRecords) {
			...
			this.param = new Params.spS_AgentAccountType_Display(true);
			...
			SPs.spS_AgentAccountType_Display SP = new SPs.spS_AgentAccountType_Display(false);

			if (SP.Execute(ref this.param, ref DS, startRecord, maxRecords)) {

				this.DataSource = DS.Tables["spS_AgentAccountType_Display"].DefaultView;
				this.DataValueField = SPs.spS_AgentAccountType_Display.Resultset1.Fields.Column_ID1.ColumnName;
				this.DataTextField = SPs.spS_AgentAccountType_Display.Resultset1.Fields.Column_Display.ColumnName;
				...
				SP.Dispose();
				throw new Evernet.MoneyExchange.DataClasses.CustomException(this.param, "WebDropDownList_AgentAccountType : System.Web.UI.WebControls.DropDownList", "RefreshData");
			}
		...
		/// <summary>
		/// Loads the AgentAccountType abstract class for the current selected record primary key.
		/// </summary>
		...
		public bool RefreshCurrentRecord() {
			...
			if (this.SelectedIndex == -1) {
				...
				if (oAbstract_AgentAccountType != null) {

					oAbstract_AgentAccountType.Reset();
				}
				return(false);
			}
			...
			if (this.oAbstract_AgentAccountType == null) {

				switch (this.LastKnownConnectionType) {

					case Evernet.MoneyExchange.DataClasses.ConnectionType.ConnectionString:
							this.oAbstract_AgentAccountType = new Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType(this.connectionString);
						break;

					case Evernet.MoneyExchange.DataClasses.ConnectionType.SqlConnection:
							this.oAbstract_AgentAccountType = new Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType(this.sqlConnection);
						break;
				}
			}

			return(this.oAbstract_AgentAccountType.Refresh(PK_Value));
		}
	}
}
