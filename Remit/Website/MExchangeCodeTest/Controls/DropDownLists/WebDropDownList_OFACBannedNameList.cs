...
namespace Evernet.MoneyExchange.Web.DropDownLists {
	...
	/// <summary>
	/// This class derives from System.Web.UI.WebControls.DropDownList class and was built to display the 'OFACBannedNameList' table content.
	/// </summary>
#if OLYMARS_ATTRIBUTE || OLYMARS_DEBUG
	[Evernet.MoneyExchange.DataClasses.OlymarsInformation(DeveloperName="Evernet Technologies LLC", GeneratedOn="2004/11/30 19:47:38", SqlObjectDependancyName="OFACBannedNameList", SqlObjectDependancyRevision=352)]
#endif
	[System.Drawing.ToolboxBitmap(typeof(WebDropDownList_OFACBannedNameList), "WebDropDownList.bmp")]
	public class WebDropDownList_OFACBannedNameList : System.Web.UI.WebControls.DropDownList {

		private Params.spS_OFACBannedNameList_Display param;
		private Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList oAbstract_OFACBannedNameList;
		...
		/// </summary>
		public Params.spS_OFACBannedNameList_Display SP_Parameter {
		...
		/// <summary>
		/// Returns the 'OFACBannedNameList' abstract class so that you can access any table
		...
		/// </summary>
		public Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList Abstract_OFACBannedNameList {

			get {

				return(this.oAbstract_OFACBannedNameList);
			}
		}
		...
		public override void Dispose() {
			...
			this.oAbstract_OFACBannedNameList = null;
		...
		/// <summary>
		/// Create a new instance of the Evernet.MoneyExchange.Web.DropDownLists.WebDropDownList_OFACBannedNameList class.
		/// </summary>
		public WebDropDownList_OFACBannedNameList() : base() {
		...
		public void Initialize(string connectionString) {
			...
			if (olymarsDebugCheck == null || (string)olymarsDebugCheck == "True") {
				...
				if (DebugConnectionString.Length != 0) {
					...
					sqlCommand.CommandText = "Select sysobjects.schema_ver from sysobjects where sysobjects.name = 'OFACBannedNameList'";
					...
					if (CurrentRevision != OriginalRevision) {
						...
						throw new System.InvalidOperationException(System.String.Format("OLYMARS: This code is not in sync anymore with [{0}]. It was generated when [{0}] version was: {2}. Current [{0}] version is: {1}{3}{3}You can either regenerate the code for this class so that it will be based on the new version or edit the configuration file of the class caller application and paste the following code:{3}{3}<?xml version=\"1.0\" encoding=\"utf-8\" ?>{3}<configuration>{3}\t<appSettings>{3}\t\t<add key=\"OlymarsDebugCheck\" value=\"False\" />{3}\t</appSettings>{3}</configuration>{3}{3}You will need to reload the caller application if it is a Windows Forms based application.", "OFACBannedNameList", CurrentRevision, OriginalRevision, System.Environment.NewLine));
					}
				}
			}
#endif
		...
		public void RefreshData(System.Data.SqlTypes.SqlGuid PK_Id, int startRecord, int maxRecords) {
			...
			this.param = new Params.spS_OFACBannedNameList_Display(true);
			...
			SPs.spS_OFACBannedNameList_Display SP = new SPs.spS_OFACBannedNameList_Display(false);

			if (SP.Execute(ref this.param, ref DS, startRecord, maxRecords)) {

				this.DataSource = DS.Tables["spS_OFACBannedNameList_Display"].DefaultView;
				this.DataValueField = SPs.spS_OFACBannedNameList_Display.Resultset1.Fields.Column_ID1.ColumnName;
				this.DataTextField = SPs.spS_OFACBannedNameList_Display.Resultset1.Fields.Column_Display.ColumnName;
				...
				SP.Dispose();
				throw new Evernet.MoneyExchange.DataClasses.CustomException(this.param, "WebDropDownList_OFACBannedNameList : System.Web.UI.WebControls.DropDownList", "RefreshData");
			}
		...
		/// <summary>
		/// Loads the OFACBannedNameList abstract class for the current selected record primary key.
		/// </summary>
		...
		public bool RefreshCurrentRecord() {
			...
			if (this.SelectedIndex == -1) {
				...
				if (oAbstract_OFACBannedNameList != null) {

					oAbstract_OFACBannedNameList.Reset();
				}
				return(false);
			}
			...
			if (this.oAbstract_OFACBannedNameList == null) {

				switch (this.LastKnownConnectionType) {

					case Evernet.MoneyExchange.DataClasses.ConnectionType.ConnectionString:
							this.oAbstract_OFACBannedNameList = new Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList(this.connectionString);
						break;

					case Evernet.MoneyExchange.DataClasses.ConnectionType.SqlConnection:
							this.oAbstract_OFACBannedNameList = new Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList(this.sqlConnection);
						break;
				}
			}

			return(this.oAbstract_OFACBannedNameList.Refresh(PK_Id));
		}
	}
}
