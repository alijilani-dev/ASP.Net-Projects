...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CommissionCurrencyType.
	/// </summary>
	public class WebForm_CommissionCurrencyType : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CommissionCurrencyType() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CommissionCurrencyType oAbstract_CommissionCurrencyType = new Evernet.MoneyExchange.AbstractClasses.Abstract_CommissionCurrencyType(ConnectionString);

			if (!oAbstract_CommissionCurrencyType.Refresh(CurrentID)) {
			...
			if (!oAbstract_CommissionCurrencyType.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CommissionCurrencyType.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CommissionCurrencyType.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_CommissionCurrencyType.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CommissionCurrencyType.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CommissionCurrencyType.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CommissionCurrencyType Param = null;
				SPs.spU_CommissionCurrencyType SP = null;

				Param = new Params.spU_CommissionCurrencyType();
				...
				SP = new SPs.spU_CommissionCurrencyType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CommissionCurrencyType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CommissionCurrencyType.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CommissionCurrencyType Param = null;
				SPs.spI_CommissionCurrencyType SP = null;

				Param = new Params.spI_CommissionCurrencyType();
				...
				SP = new SPs.spI_CommissionCurrencyType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CommissionCurrencyType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CommissionCurrencyType.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CommissionCurrencyType Param = null;
			SPs.spD_CommissionCurrencyType SP = null;

			Param = new Params.spD_CommissionCurrencyType();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_CommissionCurrencyType();
