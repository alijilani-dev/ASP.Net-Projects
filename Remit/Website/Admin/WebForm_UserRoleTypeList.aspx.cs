...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_UserRoleTypeList.
	/// </summary>
	public class WebForm_UserRoleTypeList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_UserRoleTypeList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_UserRoleTypeList oAbstract_UserRoleTypeList = new Evernet.MoneyExchange.AbstractClasses.Abstract_UserRoleTypeList(ConnectionString);

			if (!oAbstract_UserRoleTypeList.Refresh(CurrentID)) {
			...
			if (!oAbstract_UserRoleTypeList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_UserRoleTypeList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_UserRoleTypeList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_UserRoleTypeList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_UserRoleTypeList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_UserRoleTypeList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_UserRoleTypeList Param = null;
				SPs.spU_UserRoleTypeList SP = null;

				Param = new Params.spU_UserRoleTypeList();
				...
				SP = new SPs.spU_UserRoleTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_UserRoleTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_UserRoleTypeList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_UserRoleTypeList Param = null;
				SPs.spI_UserRoleTypeList SP = null;

				Param = new Params.spI_UserRoleTypeList();
				...
				SP = new SPs.spI_UserRoleTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_UserRoleTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_UserRoleTypeList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_UserRoleTypeList Param = null;
			SPs.spD_UserRoleTypeList SP = null;

			Param = new Params.spD_UserRoleTypeList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_UserRoleTypeList();
