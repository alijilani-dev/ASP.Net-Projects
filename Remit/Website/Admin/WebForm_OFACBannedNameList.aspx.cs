...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_OFACBannedNameList.
	/// </summary>
	public class WebForm_OFACBannedNameList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_OFACBannedNameList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList oAbstract_OFACBannedNameList = new Evernet.MoneyExchange.AbstractClasses.Abstract_OFACBannedNameList(ConnectionString);

			if (!oAbstract_OFACBannedNameList.Refresh(CurrentID)) {
			...
			if (!oAbstract_OFACBannedNameList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_OFACBannedNameList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_OFACBannedNameList.Col_Active.IsNull) {

				chk_Active.Checked = oAbstract_OFACBannedNameList.Col_Active.Value;
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_OFACBannedNameList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_OFACBannedNameList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_OFACBannedNameList Param = null;
				SPs.spU_OFACBannedNameList SP = null;

				Param = new Params.spU_OFACBannedNameList();
				...
				SP = new SPs.spU_OFACBannedNameList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_OFACBannedNameList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_OFACBannedNameList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_OFACBannedNameList Param = null;
				SPs.spI_OFACBannedNameList SP = null;

				Param = new Params.spI_OFACBannedNameList();
				...
				Param.Param_Active = new System.Data.SqlTypes.SqlBoolean(chk_Active.Checked);
				SP = new SPs.spI_OFACBannedNameList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_OFACBannedNameList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_OFACBannedNameList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_OFACBannedNameList Param = null;
			SPs.spD_OFACBannedNameList SP = null;

			Param = new Params.spD_OFACBannedNameList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_OFACBannedNameList();
