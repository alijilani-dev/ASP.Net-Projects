...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_EntityList.
	/// </summary>
	public class WebForm_EntityList : System.Web.UI.Page {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_EntityList oAbstract_EntityList = new Evernet.MoneyExchange.AbstractClasses.Abstract_EntityList(ConnectionString);

			if (!oAbstract_EntityList.Refresh(CurrentID)) {
			...
			if (!oAbstract_EntityList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_EntityList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_EntityList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_EntityList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_EntityList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_EntityList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_EntityList Param = null;
				SPs.spU_EntityList SP = null;

				Param = new Params.spU_EntityList();
				...
				SP = new SPs.spU_EntityList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_EntityList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_EntityList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_EntityList Param = null;
				SPs.spI_EntityList SP = null;

				Param = new Params.spI_EntityList();
				...
				SP = new SPs.spI_EntityList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_EntityList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_EntityList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_EntityList Param = null;
			SPs.spD_EntityList SP = null;

			Param = new Params.spD_EntityList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_EntityList();
