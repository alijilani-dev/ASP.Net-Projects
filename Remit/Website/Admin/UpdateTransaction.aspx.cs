...
namespace Evernet.MoneyExchange.Admin {
	public class UpdateTransaction : System.Web.UI.Page {
		#region Declarations.
		...
		protected System.Web.UI.WebControls.Label lblSRTRNo;
		protected System.Web.UI.WebControls.Button btnGetInfo;
		protected System.Web.UI.WebControls.TextBox txtSRTRNo;
		...
		private void Page_Load(object sender, System.EventArgs e) {
			...
			/*if(!IsPostBack) {
				if(User.IsInRole(UserRoles.Administrator.ToString())||(User.IsInRole(UserRoles.AgentLocationPayInTransactionInitiator.ToString())))
				{
		...
		private void InitializeComponent() {    
			...
			this.chbxCreateNewCustomer.CheckedChanged += new System.EventHandler(this.chbxCreateNewCustomer_CheckedChanged);
...
//				}
		...
		private void UpdateCreditInformation() {
			...
			if(li!=null) {
				//lblCreditBalance.Text = new Guid(li.Value)).ToString();
		...
		private void BindDestinationCountry() 
		{
			...
			if(li!=null) 
			{
				...
				//mrows = ds.Tables[0].Rows.Count;
				//lblMessage.Text = mrows.ToString();
				
				for(int i = 0; i < ds.Tables[0].Rows.Count; i++)
				{
					if(ds.Tables[0].Rows[i]["Display"].ToString().StartsWith("INDIA"))
					//if(ds.Tables[0].Rows[i]["Display"].ToString().StartsWith("INDIA") && ds.Tables[0].Rows[i]["Display"].ToString().Length == 10)
						break;
					else
...
//			pnlMessageToAgent.Visible=false;
			pnlBottomPart.Visible=false;
			// Check if the card exists in the database
			DataSet ds1 = SqlHelper.ExecuteDataset(ConfigHelper.ConStr,
				CommandType.Text,
				"Select Id From CustomerCardList Where Code='"+ txtCustomerCardNumber.Text +"'"); // And AgentId='"+ddlOriginAgent.SelectedValue+"'");
...
//					txtCustomerLoginName.Enabled=false;
//					txtCustomerLoginName.ReadOnly = true;

//					txtCustomerLoginName.Text = dr["LoginName"].ToString();
...
//					txtCustomerFax.Text = dr["Fax"].ToString();
...
//					lblCustomerRecordStatus.Text = "New Customer";
//					lblCustomerCardMessage.Text = "New Customer - Please provide the details of the customer below!";
//
//					butUpdateCustomerRecord.Visible=false;
			...
			UpdateVisibilityBasedOnModeOfPayment();
//			pnlMessageToAgent.Visible=true;
			pnlBottomPart.Visible=true;
		}
...
*/
		...
		private string GenerateTransactionNumber(string paymentModePrefix) 
		{
			...
			if (ddlDestinationCountry.SelectedItem.Text.StartsWith("INDIA - SPEED"))
			{
				trNum = GetZohaTransactionNumber();
			}
			else
			{
				//trNum = paymentModePrefix+"-"+DateTime.Today.ToString("ddMMyy")+"-";
		...
		/*	string trNum = String.Empty;
			...
			while(!gotUnique) {
				num = rdm1.Next(100000,999999);

				trNum += num;

				ds = SqlHelper.ExecuteDataset(ConfigHelper.ConStr,
					CommandType.Text,
					"Select TransactionNumber From TransactionDetails Where TransactionNumber='"+ trNum +"'");
...
*/
		...
		//	aoUser.Refresh(new Guid(User.Identity.Name));
			...
			//if(txtTransactionPassword.Text.Trim().ToUpper() != aoUser.Col_TransactionPassword.Value.ToUpper()) {
			//	Message = "Transaction password does not match!";
			//	lblInitateTransferMessage.Text = Message;
			//	return;
		//	}
			...
			//string question = txtBeneficeryQuestion.Text;
			//string answer = txtBeneficeryAnswer.Text;
			//string m2b = txtBeneficeryMessage.Text;
			//string m2a = txtMessageToAgent.Text;
			decimal providedExchangeRate = decimal.Parse(txtExchangeRate.Text);
			...
			else
			{
				...
				if(dsCard.Tables[0].Rows.Count>0) 
				{
					...
					catch(Exception ex)
					{
						...
						Message = "Level 1 : Could not complete the transaction! Please contact the support personnel";
						return;
					}
					finally
					{
			...
			if(payinUSD>100)
			{
				//}
				//else
				//strEMCNumber = DBNull.Value.ToString();
		...
		public string GetMaxBatchNumber()
		{
			...
			DataSet ds = SqlHelper.ExecuteDataset(ConfigHelper.ConStr,
				CommandType.Text,
				"Select coalesce(count(*),0)+38 as trows from TransactionDetails where PayOutCurrencyId In (Select Id from CurrencyList Where Code = 'Rs.')");

			string str_Query = "Select coalesce(count(*),0)+38 as trows from TransactionDetails where PayOutCurrencyId In (Select Id from CurrencyList Where Code = 'Rs.')";
		...
		#region Abandoned Scheme.
		// Customer Case 1: (Customer does not exist)..Neglected for the time being..(has been created above..)
		//						a. Generate an EMC #.
		//						b. Return EMC #.
		//			Case 2: (Customer Exists without EMC#).
		//						a. Generate an EMC #.
		//						b. Return EMC #.
		//			Case 3: (Customer Exists with EMC# and Card No.).
		//						a. Get the existing EMC#.
		//						b. Return EMC #.
		#endregion Abandoned Scheme.
		/// <summary>
		...
		private void btnGetInfo_Click(object sender, System.EventArgs e)
		{
			...
			DataSet ds = SqlHelper.ExecuteDataset(ConfigHelper.ConStr,
				CommandType.Text,
				@"Select cl.Id [ID1], cl.Name + ' [' + cl.Code + ']' [Display] From  CountryList cl join TransactionDetails td on td.PayInCurrencyId = cl.BaseCurrency
					Where AllowedInternationalTransactionType='"+ TransactionPermissionType.Send.ToString() +@"' Or AllowedInternationalTransactionType='"+ TransactionPermissionType.SendReceive.ToString() +@"'
					Or AllowedDomesticTransactionType='"+ TransactionPermissionType.Send.ToString() +"' Or AllowedDomesticTransactionType='"+ TransactionPermissionType.SendReceive.ToString() +"' and td.transactionnumber = '" + txtSRTRNo.Text + "'");
			
			//DataRow dr = ds.Tables[0].Rows[0][0]
			ddlOriginCountry.SelectedIndex = -1;
			ListItem liCountry = ddlOriginCountry.Items.FindByValue(ds.Tables[0].Rows[0][0].ToString());
			if(liCountry!=null)
				liCountry.Selected=true;
			else ddlOriginCountry.SelectedIndex = 2;
		}
	}
}