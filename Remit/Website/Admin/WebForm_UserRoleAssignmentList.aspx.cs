...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_UserRoleAssignmentList.
	/// </summary>
	public class WebForm_UserRoleAssignmentList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		protected System.Web.UI.WebControls.Label lab_Role;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_UserRoleAssignmentList() {
		...
		private void RefreshForeignTables() {
			...
			try {
				...
				com_Role.RefreshData(System.Data.SqlTypes.SqlString.Null);
			}
			catch (Evernet.MoneyExchange.DataClasses.CustomException customException) {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_UserRoleAssignmentList oAbstract_UserRoleAssignmentList = new Evernet.MoneyExchange.AbstractClasses.Abstract_UserRoleAssignmentList(ConnectionString);

			if (!oAbstract_UserRoleAssignmentList.Refresh(CurrentID)) {
			...
			com_UserId.Items.FindByValue(Convert.ToString(oAbstract_UserRoleAssignmentList.Col_UserId)).Selected = true;

			com_Role.Items.FindByValue(Convert.ToString(oAbstract_UserRoleAssignmentList.Col_Role)).Selected = true;
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_UserRoleAssignmentList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_UserRoleAssignmentList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_UserRoleAssignmentList Param = null;
				SPs.spU_UserRoleAssignmentList SP = null;

				Param = new Params.spU_UserRoleAssignmentList();
				...
				Param.Param_Role = Convert.ToString(com_Role.SelectedItem.Value);

				SP = new SPs.spU_UserRoleAssignmentList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_UserRoleAssignmentList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_UserRoleAssignmentList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_UserRoleAssignmentList Param = null;
				SPs.spI_UserRoleAssignmentList SP = null;

				Param = new Params.spI_UserRoleAssignmentList();
				...
				SP = new SPs.spI_UserRoleAssignmentList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_UserRoleAssignmentList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_UserRoleAssignmentList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_UserRoleAssignmentList Param = null;
			SPs.spD_UserRoleAssignmentList SP = null;

			Param = new Params.spD_UserRoleAssignmentList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_UserRoleAssignmentList();
