...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CustomerCardStatusList.
	/// </summary>
	public class WebForm_CustomerCardStatusList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CustomerCardStatusList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerCardStatusList oAbstract_CustomerCardStatusList = new Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerCardStatusList(ConnectionString);

			if (!oAbstract_CustomerCardStatusList.Refresh(CurrentID)) {
			...
			if (!oAbstract_CustomerCardStatusList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CustomerCardStatusList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CustomerCardStatusList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_CustomerCardStatusList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CustomerCardStatusList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CustomerCardStatusList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CustomerCardStatusList Param = null;
				SPs.spU_CustomerCardStatusList SP = null;

				Param = new Params.spU_CustomerCardStatusList();
				...
				SP = new SPs.spU_CustomerCardStatusList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerCardStatusList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerCardStatusList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CustomerCardStatusList Param = null;
				SPs.spI_CustomerCardStatusList SP = null;

				Param = new Params.spI_CustomerCardStatusList();
				...
				SP = new SPs.spI_CustomerCardStatusList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerCardStatusList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerCardStatusList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CustomerCardStatusList Param = null;
			SPs.spD_CustomerCardStatusList SP = null;

			Param = new Params.spD_CustomerCardStatusList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_CustomerCardStatusList();
