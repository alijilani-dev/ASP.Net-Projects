...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_AgentAccountType.
	/// </summary>
	public class WebForm_AgentAccountType : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_AgentAccountType() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType oAbstract_AgentAccountType = new Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAccountType(ConnectionString);

			if (!oAbstract_AgentAccountType.Refresh(CurrentID)) {
			...
			if (!oAbstract_AgentAccountType.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_AgentAccountType.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_AgentAccountType.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_AgentAccountType.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_AgentAccountType.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_AgentAccountType.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_AgentAccountType Param = null;
				SPs.spU_AgentAccountType SP = null;

				Param = new Params.spU_AgentAccountType();
				...
				SP = new SPs.spU_AgentAccountType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentAccountType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentAccountType.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_AgentAccountType Param = null;
				SPs.spI_AgentAccountType SP = null;

				Param = new Params.spI_AgentAccountType();
				...
				SP = new SPs.spI_AgentAccountType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentAccountType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentAccountType.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_AgentAccountType Param = null;
			SPs.spD_AgentAccountType SP = null;

			Param = new Params.spD_AgentAccountType();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_AgentAccountType();
