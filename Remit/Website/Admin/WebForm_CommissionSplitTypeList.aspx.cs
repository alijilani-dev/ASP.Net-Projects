...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CommissionSplitTypeList.
	/// </summary>
	public class WebForm_CommissionSplitTypeList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CommissionSplitTypeList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CommissionSplitTypeList oAbstract_CommissionSplitTypeList = new Evernet.MoneyExchange.AbstractClasses.Abstract_CommissionSplitTypeList(ConnectionString);

			if (!oAbstract_CommissionSplitTypeList.Refresh(CurrentID)) {
			...
			if (!oAbstract_CommissionSplitTypeList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CommissionSplitTypeList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CommissionSplitTypeList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_CommissionSplitTypeList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CommissionSplitTypeList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CommissionSplitTypeList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CommissionSplitTypeList Param = null;
				SPs.spU_CommissionSplitTypeList SP = null;

				Param = new Params.spU_CommissionSplitTypeList();
				...
				SP = new SPs.spU_CommissionSplitTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CommissionSplitTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CommissionSplitTypeList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CommissionSplitTypeList Param = null;
				SPs.spI_CommissionSplitTypeList SP = null;

				Param = new Params.spI_CommissionSplitTypeList();
				...
				SP = new SPs.spI_CommissionSplitTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CommissionSplitTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CommissionSplitTypeList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CommissionSplitTypeList Param = null;
			SPs.spD_CommissionSplitTypeList SP = null;

			Param = new Params.spD_CommissionSplitTypeList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_CommissionSplitTypeList();
