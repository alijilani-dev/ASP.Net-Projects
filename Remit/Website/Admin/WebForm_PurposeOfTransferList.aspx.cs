...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_PurposeOfTransferList.
	/// </summary>
	public class WebForm_PurposeOfTransferList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		protected System.Web.UI.WebControls.Label lab_PaymentModeBaseType;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_PurposeOfTransferList() {
			...
			*/
			...
			if (Request.Params["Action"] != null) {
				...
				if (!Page.IsPostBack) {
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.PurposeOfTransferCreator.ToString())
						&&
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.PurposeOfTransferEditor.ToString())
						&&
						!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.Administrator.ToString())
						) {
		...
		private void RefreshForeignTables() {
			...
			try {
				...
				com_PaymentModeBaseType.RefreshData(System.Data.SqlTypes.SqlString.Null);
			}
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_PurposeOfTransferList oAbstract_PurposeOfTransferList = new Evernet.MoneyExchange.AbstractClasses.Abstract_PurposeOfTransferList(ConnectionString);

			if (!oAbstract_PurposeOfTransferList.Refresh(CurrentID)) {
			...
			if (!oAbstract_PurposeOfTransferList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_PurposeOfTransferList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_PurposeOfTransferList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_PurposeOfTransferList.Col_Value.Value.ToString();
			}
			else {
			...
			com_PaymentModeBaseType.Items.FindByValue(Convert.ToString(oAbstract_PurposeOfTransferList.Col_PaymentModeBaseType)).Selected = true;
		...
		private void EmptyControls() {
			...
			txt_Name.Text = String.Empty;
			txt_Value.Text = String.Empty;
			com_PaymentModeBaseType.SelectedIndex = 0;
		}
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_PurposeOfTransferList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_PurposeOfTransferList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_PurposeOfTransferList Param = null;
				SPs.spU_PurposeOfTransferList SP = null;

				Param = new Params.spU_PurposeOfTransferList();
				...
				Param.Param_PaymentModeBaseType = Convert.ToString(com_PaymentModeBaseType.SelectedItem.Value);

				SP = new SPs.spU_PurposeOfTransferList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_PurposeOfTransferList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_PurposeOfTransferList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_PurposeOfTransferList Param = null;
				SPs.spI_PurposeOfTransferList SP = null;

				Param = new Params.spI_PurposeOfTransferList();
				...
				SP = new SPs.spI_PurposeOfTransferList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_PurposeOfTransferList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_PurposeOfTransferList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_PurposeOfTransferList Param = null;
			SPs.spD_PurposeOfTransferList SP = null;

			Param = new Params.spD_PurposeOfTransferList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_PurposeOfTransferList();
