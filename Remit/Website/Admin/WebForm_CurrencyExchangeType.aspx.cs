...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CurrencyExchangeType.
	/// </summary>
	public class WebForm_CurrencyExchangeType : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CurrencyExchangeType() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CurrencyExchangeType oAbstract_CurrencyExchangeType = new Evernet.MoneyExchange.AbstractClasses.Abstract_CurrencyExchangeType(ConnectionString);

			if (!oAbstract_CurrencyExchangeType.Refresh(CurrentID)) {
			...
			if (!oAbstract_CurrencyExchangeType.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CurrencyExchangeType.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CurrencyExchangeType.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_CurrencyExchangeType.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CurrencyExchangeType.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CurrencyExchangeType.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CurrencyExchangeType Param = null;
				SPs.spU_CurrencyExchangeType SP = null;

				Param = new Params.spU_CurrencyExchangeType();
				...
				SP = new SPs.spU_CurrencyExchangeType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CurrencyExchangeType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CurrencyExchangeType.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CurrencyExchangeType Param = null;
				SPs.spI_CurrencyExchangeType SP = null;

				Param = new Params.spI_CurrencyExchangeType();
				...
				SP = new SPs.spI_CurrencyExchangeType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CurrencyExchangeType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CurrencyExchangeType.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CurrencyExchangeType Param = null;
			SPs.spD_CurrencyExchangeType SP = null;

			Param = new Params.spD_CurrencyExchangeType();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_CurrencyExchangeType();
