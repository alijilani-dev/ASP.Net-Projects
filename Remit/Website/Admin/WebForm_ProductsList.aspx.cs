...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_ProductsList.
	/// </summary>
	public class WebForm_ProductsList : System.Web.UI.Page {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_ProductsList oAbstract_ProductsList = new Evernet.MoneyExchange.AbstractClasses.Abstract_ProductsList(ConnectionString);

			if (!oAbstract_ProductsList.Refresh(CurrentID)) {
			...
			if (!oAbstract_ProductsList.Col_Id.IsNull) {

				txt_Id.Text = oAbstract_ProductsList.Col_Id.Value.ToString();
			}
			else {
			...
			if (!oAbstract_ProductsList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_ProductsList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_ProductsList.Col_Points.IsNull) {

				txt_Points.Text = oAbstract_ProductsList.Col_Points.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_ProductsList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_ProductsList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_ProductsList Param = null;
				SPs.spU_ProductsList SP = null;

				Param = new Params.spU_ProductsList();
				...
				SP = new SPs.spU_ProductsList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_ProductsList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_ProductsList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_ProductsList Param = null;
				SPs.spI_ProductsList SP = null;

				Param = new Params.spI_ProductsList();
				...
				SP = new SPs.spI_ProductsList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_ProductsList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_ProductsList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_ProductsList Param = null;
			SPs.spD_ProductsList SP = null;

			Param = new Params.spD_ProductsList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_ProductsList();
