...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CustomerLoyaltyPointsAccumulationHistory.
	/// </summary>
	public class WebForm_CustomerLoyaltyPointsAccumulationHistory : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CustomerLoyaltyPointsAccumulationHistory() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerLoyaltyPointsAccumulationHistory oAbstract_CustomerLoyaltyPointsAccumulationHistory = new Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerLoyaltyPointsAccumulationHistory(ConnectionString);

			if (!oAbstract_CustomerLoyaltyPointsAccumulationHistory.Refresh(CurrentID)) {
			...
			com_CustomerId.Items.FindByValue(Convert.ToString(oAbstract_CustomerLoyaltyPointsAccumulationHistory.Col_CustomerId)).Selected = true;

			com_TransactionId.Items.FindByValue(Convert.ToString(oAbstract_CustomerLoyaltyPointsAccumulationHistory.Col_TransactionId)).Selected = true;

			if (!oAbstract_CustomerLoyaltyPointsAccumulationHistory.Col_Points.IsNull) {

				txt_Points.Text = oAbstract_CustomerLoyaltyPointsAccumulationHistory.Col_Points.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CustomerLoyaltyPointsAccumulationHistory Param = null;
				SPs.spU_CustomerLoyaltyPointsAccumulationHistory SP = null;

				Param = new Params.spU_CustomerLoyaltyPointsAccumulationHistory();
				...
				SP = new SPs.spU_CustomerLoyaltyPointsAccumulationHistory();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CustomerLoyaltyPointsAccumulationHistory Param = null;
				SPs.spI_CustomerLoyaltyPointsAccumulationHistory SP = null;

				Param = new Params.spI_CustomerLoyaltyPointsAccumulationHistory();
				...
				SP = new SPs.spI_CustomerLoyaltyPointsAccumulationHistory();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerLoyaltyPointsAccumulationHistory.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CustomerLoyaltyPointsAccumulationHistory Param = null;
			SPs.spD_CustomerLoyaltyPointsAccumulationHistory SP = null;

			Param = new Params.spD_CustomerLoyaltyPointsAccumulationHistory();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_CustomerLoyaltyPointsAccumulationHistory();
