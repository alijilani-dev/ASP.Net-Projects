...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_MasterModuleList.
	/// </summary>
	public class WebForm_MasterModuleList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_MasterModuleList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_MasterModuleList oAbstract_MasterModuleList = new Evernet.MoneyExchange.AbstractClasses.Abstract_MasterModuleList(ConnectionString);

			if (!oAbstract_MasterModuleList.Refresh(CurrentID)) {
			...
			if (!oAbstract_MasterModuleList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_MasterModuleList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_MasterModuleList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_MasterModuleList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_MasterModuleList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_MasterModuleList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_MasterModuleList Param = null;
				SPs.spU_MasterModuleList SP = null;

				Param = new Params.spU_MasterModuleList();
				...
				SP = new SPs.spU_MasterModuleList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_MasterModuleList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_MasterModuleList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_MasterModuleList Param = null;
				SPs.spI_MasterModuleList SP = null;

				Param = new Params.spI_MasterModuleList();
				...
				SP = new SPs.spI_MasterModuleList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_MasterModuleList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_MasterModuleList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_MasterModuleList Param = null;
			SPs.spD_MasterModuleList SP = null;

			Param = new Params.spD_MasterModuleList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_MasterModuleList();
