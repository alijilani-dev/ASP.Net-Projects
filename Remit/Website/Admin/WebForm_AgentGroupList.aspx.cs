...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_AgentGroupList.
	/// </summary>
	public class WebForm_AgentGroupList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		protected System.Web.UI.WebControls.Label lab_PrefixCode;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
		protected System.Web.UI.WebControls.TextBox txt_PrefixCode;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
		protected System.Web.UI.WebControls.Label labError_PrefixCode;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
			...
			*/
			...
			if (Request.Params["Action"] != null) {
				...
				if (!Page.IsPostBack) {
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.AgentGroupCreator.ToString())
						&&
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.AgentGroupEditor.ToString())
						&&
						!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.Administrator.ToString())
						) {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_AgentGroupList oAbstract_AgentGroupList = new Evernet.MoneyExchange.AbstractClasses.Abstract_AgentGroupList(ConnectionString);

			if (!oAbstract_AgentGroupList.Refresh(CurrentID)) {
			...
			com_AgentAccountId.Items.FindByValue(Convert.ToString(oAbstract_AgentGroupList.Col_AgentAccountId)).Selected = true;

			if (!oAbstract_AgentGroupList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_AgentGroupList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_AgentGroupList.Col_PrefixCode.IsNull) {

				txt_PrefixCode.Text = oAbstract_AgentGroupList.Col_PrefixCode.Value.ToString();
			}
			else {

				txt_PrefixCode.Text = String.Empty;
			}
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_AgentGroupList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_AgentGroupList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_AgentGroupList Param = null;
				SPs.spU_AgentGroupList SP = null;

				Param = new Params.spU_AgentGroupList();
				...
				if (txt_PrefixCode.Text.Trim() != String.Empty) {

					Param.Param_PrefixCode = new System.Data.SqlTypes.SqlString(txt_PrefixCode.Text);
				}

				SP = new SPs.spU_AgentGroupList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentGroupList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentGroupList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_AgentGroupList Param = null;
				SPs.spI_AgentGroupList SP = null;

				Param = new Params.spI_AgentGroupList();
				...
				SP = new SPs.spI_AgentGroupList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentGroupList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentGroupList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_AgentGroupList Param = null;
			SPs.spD_AgentGroupList SP = null;

			Param = new Params.spD_AgentGroupList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_AgentGroupList();
