...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CurrencyList.
	/// </summary>
	public class WebForm_CurrencyList : System.Web.UI.Page {
		...
		protected System.Web.UI.WebControls.TextBox txt_Scale;
		/// <summary>
		/// [To be supplied.]
		/// </summary>
			...
			*/
			...
			if (Request.Params["Action"] != null) {
				...
				if (!Page.IsPostBack) {
					...
					if(!User.IsInRole(UserRoles.CurrencyManagerCreator.ToString())
						&&
					...
					if(!User.IsInRole(UserRoles.CurrencyManagerEditor.ToString())&&
						!User.IsInRole(UserRoles.Administrator.ToString())) {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CurrencyList oAbstract_CurrencyList = new Evernet.MoneyExchange.AbstractClasses.Abstract_CurrencyList(ConnectionString);

			if (!oAbstract_CurrencyList.Refresh(CurrentID)) {
			...
			if (!oAbstract_CurrencyList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CurrencyList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CurrencyList.Col_Code.IsNull) {

				txt_Code.Text = oAbstract_CurrencyList.Col_Code.Value.ToString();
			}
			else {
			...
			if(!oAbstract_CurrencyList.Col_Scale.IsNull) {
				txt_Scale.Text = oAbstract_CurrencyList.Col_Scale.Value.ToString();
			} else {
				txt_Scale.Text = String.Empty;
			}
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CurrencyList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CurrencyList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CurrencyList Param = null;
				SPs.spU_CurrencyList SP = null;

				Param = new Params.spU_CurrencyList();
				...
				if(txt_Scale.Text.Trim() != String.Empty) {
					Param.Param_Scale = Convert.ToByte(txt_Scale.Text);
				}

				SP = new SPs.spU_CurrencyList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CurrencyList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CurrencyList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CurrencyList Param = null;
				SPs.spI_CurrencyList SP = null;

				Param = new Params.spI_CurrencyList();
				...
				SP = new SPs.spI_CurrencyList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CurrencyList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CurrencyList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CurrencyList Param = null;
			SPs.spD_CurrencyList SP = null;

			Param = new Params.spD_CurrencyList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_CurrencyList();
