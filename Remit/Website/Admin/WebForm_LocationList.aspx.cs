...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_LocationList.
	/// </summary>
	public class WebForm_LocationList : System.Web.UI.Page {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_LocationList oAbstract_LocationList = new Evernet.MoneyExchange.AbstractClasses.Abstract_LocationList(ConnectionString);

			if (!oAbstract_LocationList.Refresh(CurrentID)) {
			...
			if (!oAbstract_LocationList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_LocationList.Col_Name.Value.ToString();
			}
			else {
			...
			com_CountryId.Items.FindByValue(Convert.ToString(oAbstract_LocationList.Col_CountryId)).Selected = true;
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_LocationList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_LocationList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_LocationList Param = null;
				SPs.spU_LocationList SP = null;

				Param = new Params.spU_LocationList();
				...
				SP = new SPs.spU_LocationList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_LocationList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_LocationList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_LocationList Param = null;
				SPs.spI_LocationList SP = null;

				Param = new Params.spI_LocationList();
				...
				SP = new SPs.spI_LocationList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_LocationList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_LocationList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_LocationList Param = null;
			SPs.spD_LocationList SP = null;

			Param = new Params.spD_LocationList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_LocationList();
