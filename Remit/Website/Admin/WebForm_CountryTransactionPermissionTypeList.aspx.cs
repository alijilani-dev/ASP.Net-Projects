...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CountryTransactionPermissionTypeList.
	/// </summary>
	public class WebForm_CountryTransactionPermissionTypeList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CountryTransactionPermissionTypeList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CountryTransactionPermissionTypeList oAbstract_CountryTransactionPermissionTypeList = new Evernet.MoneyExchange.AbstractClasses.Abstract_CountryTransactionPermissionTypeList(ConnectionString);

			if (!oAbstract_CountryTransactionPermissionTypeList.Refresh(CurrentID)) {
			...
			if (!oAbstract_CountryTransactionPermissionTypeList.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_CountryTransactionPermissionTypeList.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CountryTransactionPermissionTypeList.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_CountryTransactionPermissionTypeList.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CountryTransactionPermissionTypeList Param = null;
				SPs.spU_CountryTransactionPermissionTypeList SP = null;

				Param = new Params.spU_CountryTransactionPermissionTypeList();
				...
				SP = new SPs.spU_CountryTransactionPermissionTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CountryTransactionPermissionTypeList Param = null;
				SPs.spI_CountryTransactionPermissionTypeList SP = null;

				Param = new Params.spI_CountryTransactionPermissionTypeList();
				...
				SP = new SPs.spI_CountryTransactionPermissionTypeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CountryTransactionPermissionTypeList.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CountryTransactionPermissionTypeList Param = null;
			SPs.spD_CountryTransactionPermissionTypeList SP = null;

			Param = new Params.spD_CountryTransactionPermissionTypeList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_CountryTransactionPermissionTypeList();
