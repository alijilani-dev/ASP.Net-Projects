...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_AgentAvailablePaymentModeList.
	/// </summary>
	public class WebForm_AgentAvailablePaymentModeList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_AgentAvailablePaymentModeList() {
			...
			*/
			...
			if (Request.Params["Action"] != null) {
				...
				if (!Page.IsPostBack) {
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.AgentModeOfPaymentCreator.ToString())
						&&
					...
					if(!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.AgentModeOfPaymentEditor.ToString())
						&&
						!User.IsInRole(Evernet.MoneyExchange.BusinessLogic.UserRoles.Administrator.ToString())
						) {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAvailablePaymentModeList oAbstract_AgentAvailablePaymentModeList = new Evernet.MoneyExchange.AbstractClasses.Abstract_AgentAvailablePaymentModeList(ConnectionString);

			if (!oAbstract_AgentAvailablePaymentModeList.Refresh(CurrentID)) {
			...
			com_AgentId.Items.FindByValue(Convert.ToString(oAbstract_AgentAvailablePaymentModeList.Col_AgentId)).Selected = true;

			com_PaymentMode.Items.FindByValue(Convert.ToString(oAbstract_AgentAvailablePaymentModeList.Col_PaymentMode)).Selected = true;
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_AgentAvailablePaymentModeList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_AgentAvailablePaymentModeList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_AgentAvailablePaymentModeList Param = null;
				SPs.spU_AgentAvailablePaymentModeList SP = null;

				Param = new Params.spU_AgentAvailablePaymentModeList();
				...
				SP = new SPs.spU_AgentAvailablePaymentModeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentAvailablePaymentModeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentAvailablePaymentModeList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_AgentAvailablePaymentModeList Param = null;
				SPs.spI_AgentAvailablePaymentModeList SP = null;

				Param = new Params.spI_AgentAvailablePaymentModeList();
				...
				SP = new SPs.spI_AgentAvailablePaymentModeList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_AgentAvailablePaymentModeList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_AgentAvailablePaymentModeList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_AgentAvailablePaymentModeList Param = null;
			SPs.spD_AgentAvailablePaymentModeList SP = null;

			Param = new Params.spD_AgentAvailablePaymentModeList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_AgentAvailablePaymentModeList();
