...
namespace Evernet.MoneyExchange.Admin {
	...
	public class ViewReport_PrincipleCommissionUnPaid : System.Web.UI.Page {
		...
		private void BindAccountDetails() {
			...
			if(li!=null) {
				...
				query = "Select * From fn_getUnPaidPrincipleAccountDetailsForCurrency('"+ li.Value +"') Where FinalDate < '"+ startDate.AddDays(1).ToString() +"'";
				...
				query = "Select * From fn_getUnPaidPrincipleAccountDetailsForCurrency('"+ li.Value +"') Where FinalDate Between '"+ startDate.ToString() +"' And '"+ endDate.ToString() +"'";
				...
				string url = "PrintView_PrincipleCommissionUnPaid.aspx?CurrencyId="+ li.Value +"&StartDate="+ txtStartDate.Text +"&EndDate="+ txtEndDate.Text;
