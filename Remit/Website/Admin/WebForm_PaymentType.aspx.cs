...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_PaymentType.
	/// </summary>
	public class WebForm_PaymentType : System.Web.UI.Page {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_PaymentType oAbstract_PaymentType = new Evernet.MoneyExchange.AbstractClasses.Abstract_PaymentType(ConnectionString);

			if (!oAbstract_PaymentType.Refresh(CurrentID)) {
			...
			if (!oAbstract_PaymentType.Col_Name.IsNull) {

				txt_Name.Text = oAbstract_PaymentType.Col_Name.Value.ToString();
			}
			else {
			...
			if (!oAbstract_PaymentType.Col_Value.IsNull) {

				txt_Value.Text = oAbstract_PaymentType.Col_Value.Value.ToString();
			}
			else {
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_PaymentType.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_PaymentType.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_PaymentType Param = null;
				SPs.spU_PaymentType SP = null;

				Param = new Params.spU_PaymentType();
				...
				SP = new SPs.spU_PaymentType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_PaymentType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_PaymentType.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_PaymentType Param = null;
				SPs.spI_PaymentType SP = null;

				Param = new Params.spI_PaymentType();
				...
				SP = new SPs.spI_PaymentType();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_PaymentType.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Value.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_PaymentType.aspx?Action=Edit&ID={0}", Param.Param_Value.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_PaymentType Param = null;
			SPs.spD_PaymentType SP = null;

			Param = new Params.spD_PaymentType();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Value = CurrentID;

			SP = new SPs.spD_PaymentType();
