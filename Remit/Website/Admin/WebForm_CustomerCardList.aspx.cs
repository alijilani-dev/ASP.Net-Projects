...
namespace Evernet.MoneyExchange.Web.Forms {
	...
	/// <summary>
	/// Summary description for WebForm_CustomerCardList.
	/// </summary>
	public class WebForm_CustomerCardList : System.Web.UI.Page {
		...
		/// [To be supplied.]
		/// </summary>
		public WebForm_CustomerCardList() {
		...
		private void RefreshRecord() {
			...
			Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerCardList oAbstract_CustomerCardList = new Evernet.MoneyExchange.AbstractClasses.Abstract_CustomerCardList(ConnectionString);

			if (!oAbstract_CustomerCardList.Refresh(CurrentID)) {
			...
			if (!oAbstract_CustomerCardList.Col_Code.IsNull) {

				txt_Code.Text = oAbstract_CustomerCardList.Col_Code.Value.ToString();
			}
			else {
			...
			if (!oAbstract_CustomerCardList.Col_AgentId.IsNull) {

				com_AgentId.Items.FindByValue(Convert.ToString(oAbstract_CustomerCardList.Col_AgentId)).Selected = true;
			}
			else {
			...
			com_Status.Items.FindByValue(Convert.ToString(oAbstract_CustomerCardList.Col_Status)).Selected = true;
		...
		private void cmdAdd_Click(object sender, System.EventArgs e) {
			...
			if (ReturnURL.Visible) {
				...
				Response.Redirect(String.Format("WebForm_CustomerCardList.aspx?Action=Add&ReturnToUrl={0}&ReturnToDisplay={1}", ReturnURL.NavigateUrl, ReturnURL.Text));
			}
			else {

				Response.Redirect("WebForm_CustomerCardList.aspx?Action=Add");
			}
		}
		...
		private void cmdUpdate_Click(object sender, System.EventArgs e) {
			...
			if (Action == ActionEnum.Edit) {
				...
				Params.spU_CustomerCardList Param = null;
				SPs.spU_CustomerCardList SP = null;

				Param = new Params.spU_CustomerCardList();
				...
				SP = new SPs.spU_CustomerCardList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerCardList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", CurrentID.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerCardList.aspx?Action=Edit&ID={0}", CurrentID.ToString()));
					}
					return;
				}
				else {
				...
				Params.spI_CustomerCardList Param = null;
				SPs.spI_CustomerCardList SP = null;

				Param = new Params.spI_CustomerCardList();
				...
				SP = new SPs.spI_CustomerCardList();

				if (SP.Execute(ref Param)) {

					if (ReturnURL.Visible) {

						Response.Redirect(String.Format("WebForm_CustomerCardList.aspx?Action=Edit&ID={0}&ReturnToUrl={1}&ReturnToDisplay={2}", Param.Param_Id.ToString(), ReturnURL.NavigateUrl, ReturnURL.Text));
					}
					else {

						Response.Redirect(String.Format("WebForm_CustomerCardList.aspx?Action=Edit&ID={0}", Param.Param_Id.ToString()));
					}
					return;
				}
				else {
		...
		private void cmdDelete_Click(object sender, System.EventArgs e) {
			...
			Params.spD_CustomerCardList Param = null;
			SPs.spD_CustomerCardList SP = null;

			Param = new Params.spD_CustomerCardList();

			Param.SetUpConnection(ConnectionString);

			Param.Param_Id = CurrentID;

			SP = new SPs.spD_CustomerCardList();
