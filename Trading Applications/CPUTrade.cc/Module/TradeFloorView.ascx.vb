    ...
    Private Sub DGTradeFloorView_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DGTradeFloorView.ItemDataBound
        ...
        If e.Item.ItemIndex >= 0 And e.Item.Cells(e.Item.Cells.Count - 1).Text <> "3" Then 'means not comment 
            ...
            If (DataBinder.Eval(e.Item.DataItem, "BUYSELL").ToString().Trim() = "BUY") Then
                ...
                nCounter = nCounter + 1
            ElseIf (DataBinder.Eval(e.Item.DataItem, "ShippingTerms").ToString().Substring(0, 10).Trim() = "ASK For De") Then
                e.Item.FindControl("trShippingTerms").Visible = False
                nCounter = nCounter + 1
            End If
    ...
    Public Function getPostingTime() As String
        ...
        Select Case ddlLastPostings.SelectedItem.Value
            Case -1 'All
                Return ""
            Case 0 'Today
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) = 0 ) "
            Case 1 'Yesterday and Today
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 1 ) "
            Case 3 'Last 3 Days
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 3 ) "
            Case 7 'Last Weeks
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 7 ) "
            Case 10 'Last 10 days
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 10 ) "
            Case 14 'Last 2 Weeks
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 14 ) "
            Case 21 'Last 3 Weeks
                Return " ( DATEDIFF ( day , Trade_Floor.[timestamp] , getdate()) <= 21 ) "
            Case 101 'Last Months
                Return " ( DATEDIFF ( month , Trade_Floor.[timestamp] , getdate()) <= 1 ) "
            Case 102 'Last 3 Months
                Return " ( DATEDIFF ( month , Trade_Floor.[timestamp] , getdate()) <= 3 ) "
            Case 150 'Recent Postings
                Return " ( 1=1 ) "
        End Select
...
#End Region
    ...
    Private Sub DGTradeFloorView_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles DGTradeFloorView.PreRender
        ...
        DGTradeFloorView.Items(0).Cells(14).Style.Add("BORDER-TOP", "#FFB154 2px solid")
        ...
        If Showcomment = True Then
            For Each dgItem In dgMonitor.Items
                If dgItem.ItemIndex >= 0 Then
                    dgItem.Cells(2).BackColor = System.Drawing.Color.FromArgb(255, 200, 132)                  ' RGB(255, 177, 84) 'System.Drawing.Color.Khaki  '#FFB154
                    If dgItem.Cells(dgItem.Cells.Count - 1).Text = "3" Then 'means comment
                        ...
                        dgItem.Cells(13).FindControl("Table_Detail").Visible = False        ' Change
                    ...
                    End If
                    pnl = CType(dgItem.Cells(dgItem.Cells.Count - 2).FindControl("Panel1"), Panel)
                    pnl.Visible = True
                End If
            Next
            Exit Sub
    ...
    '    For Each dgItem In dgMonitor.Items
    '        l = dgItem.Cells(13).FindControl("Label10")
    ...
    '                dgItem.Cells(13).FindControl("Label10").Visible = False
