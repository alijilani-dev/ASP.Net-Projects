...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim dt_main_link As New DataTable
        Response.Cache.SetCacheability(HttpCacheability.NoCache)
        ' get the current Link ID from query string 
        ' and set the Session Variable 
        ' cureent Main_Link_ID
        Session(Session_str_Portal_ID) = 1
        pi_main_link_ID = Request.QueryString("Main_Links_ID")
        pi_Sub_link_ID = Request.QueryString("Sub_Links_ID")
        pi_WebPath = ""
        TradeMainCtrl.Main_Links_ID = pi_main_link_ID
        TradeMainCtrl.Sub_Links_ID = pi_Sub_link_ID

        If pi_main_link_ID > 0 Then
            dt_main_link = pi_obj_mainlinks.GetMainLinks(pi_main_link_ID)
            TradeMainCtrl.Redirect_Main_link_ID = "0" & dt_main_link.Rows(0).Item("Redirect_Main_Link_ID")
        End If

        If TradeMainCtrl.Redirect_Main_link_ID > 0 And Session(Session_str_UserName) <> "" Then
            Response.Redirect("PortalDefault.aspx?Main_Links_ID=" & TradeMainCtrl.Redirect_Main_link_ID)
        End If
        'ShowLogo()
        'ShowBanner()
        'ShowMainLinks()
        ...
        'ShowRightPane()
        'ShowFooter()
        'Response.Write("--- Start-" & Request.QueryString("abc") & " -- end")
    End Sub
