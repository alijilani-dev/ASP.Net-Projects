...
if len(Session("sLogmein"))=0 then
	...
	ManufCode = conn.Execute("select Max(ManufCode) from mManufacturer")(0)
	ManufCode = ManufCode & ""
	if len(ManufCode)=0 then 
		ManufCode=1 
	else
		ManufCode = cdbl(ManufCode) +1
	end if
...
if len(imgFile & "")>0 and Right(imgFile,4)<>".bin" then		
	...
	conn.BeginTrans
	ObjRs1.Open "select * from mManufacturer",conn,3,3
	ObjRs1.AddNew 	
	ObjRs1("ManufCode") = ManufCode
	ObjRs1("ManufName") = strManufName
	ObjRs1("ActDate") = now()
	...
	conn.CommitTrans
 	if err.number<>0 then
		...
		Finished="yes"
		Response.Redirect "Manufacturer.asp?mode=1"
	end if 	
%> 
