...
<script language="JavaScript1.2" type="text/JavaScript1.2" src="Main.js"></script>
<HTML>
...
<input type="hidden" name="hdMember" value="">
		...
		function move()
		{
			document.frm.action = "ImageUpload1.asp";
			document.frm.submit();
		}
          ...
          <td><table width=100% border=0 align=Center class="Border">
                ...
                <tr Class=TableBackGround> 
                  ...
                  <td width="63%"> <select name="cboMembName" style="width:250px">
                      <%
sSQL = "select Member_Company,Member_id from Members order by Member_Company"
if rsMain.State then rsMain.Close 
rsMain.Open sSQL,conn,2,1
do while not rsMain.EOF 
%>
                      <option value="<%=rsMain("Member_id")%>"><%=rsMain("Member_Company")%></option>
                      <%
rsMain.MoveNext 
loop
rsMain.Close
...
document.frm.cboMembName.selectedIndex = i;
    ...
    rsTemp.Open "Select * from Members where member_id='" & strSelect & "'",conn,3,2
...
%>
                ...
                <tr Class=TableBackGround> 
                  ...
                  <td> 
                    <%
valDb= rsTemp("Company_Logo_Url_Main") & ""
'imgA = "http://" &  Request.ServerVariables("Server_Name")
...
%>
      ...
      <form name="frmImage" method="post" encType="multipart/form-data" action="UploadNo.asp" onSubmit="JavaScript:return FormSubmit();">
...
function FormSubmit()
{
	...
	{
		document.frmImage.action="UploadNo.asp"
