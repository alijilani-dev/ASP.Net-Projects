...
Public Class EventsDB
  ...
  Public Function AddEvent(ByVal moduleId As Integer, ByVal itemId As Integer, ByVal userName As String, ByVal title As String, ByVal expireDate As DateTime, ByVal description As String, ByVal wherewhen As String, ByVal imageUrl As String, ByVal imageWidth As String, ByVal imageHeight As String) As Integer
    If (userName.Length < 1) Then
      userName = "unknown"
    End If
    Dim connection1 As New SqlConnection(ConfigurationSettings.AppSettings.Item("connectionString"))
    Dim command1 As New SqlCommand("pmAddEvent", connection1)
    ...
    Dim parameter4 As New SqlParameter("@Title", SqlDbType.NVarChar, 100)
    parameter4.Value = title
    command1.Parameters.Add(parameter4)
    Dim parameter5 As New SqlParameter("@WhereWhen", SqlDbType.NVarChar, 100)
    parameter5.Value = wherewhen
  ...
  Public Sub DeleteEvent(ByVal itemID As Integer)
    Dim connection1 As New SqlConnection(ConfigurationSettings.AppSettings.Item("connectionString"))
    Dim command1 As New SqlCommand("pmDeleteEvent", connection1)
    command1.CommandType = CommandType.StoredProcedure
    Dim parameter1 As New SqlParameter("@ItemID", SqlDbType.Int, 4)
  ...
  Public Function GetEvents(ByVal moduleId As Integer) As DataSet
    Dim connection1 As New SqlConnection(ConfigurationSettings.AppSettings.Item("connectionString"))
    Dim adapter1 As New SqlDataAdapter("pmGetEvents", connection1)
  ...
  Public Function GetSingleEvent(ByVal itemId As Integer) As SqlDataReader
    Dim connection1 As New SqlConnection(ConfigurationSettings.AppSettings.Item("connectionString"))
    Dim command1 As New SqlCommand("pmGetSingleEvent", connection1)
    command1.CommandType = CommandType.StoredProcedure
    Dim parameter1 As New SqlParameter("@ItemId", SqlDbType.Int, 4)
    parameter1.Value = itemId
    command1.Parameters.Add(parameter1)
  ...
  Public Sub UpdateEvent(ByVal moduleId As Integer, ByVal itemId As Integer, ByVal userName As String, ByVal title As String, ByVal expireDate As DateTime, ByVal description As String, ByVal wherewhen As String, ByVal imageUrl As String, ByVal imageWidth As String, ByVal imageHeight As String)
    If (userName.Length < 1) Then
      userName = "unknown"
    End If
    Dim connection1 As New SqlConnection(ConfigurationSettings.AppSettings.Item("connectionString"))
    Dim command1 As New SqlCommand("pmUpdateEvent", connection1)
    command1.CommandType = CommandType.StoredProcedure
    Dim parameter1 As New SqlParameter("@ItemID", SqlDbType.Int, 4)
    parameter1.Value = itemId
    command1.Parameters.Add(parameter1)
    ...
    command1.Parameters.Add(parameter2)
    Dim parameter3 As New SqlParameter("@Title", SqlDbType.NVarChar, 100)
    parameter3.Value = title
    command1.Parameters.Add(parameter3)
    Dim parameter4 As New SqlParameter("@WhereWhen", SqlDbType.NVarChar, 100)
    parameter4.Value = wherewhen
    command1.Parameters.Add(parameter4)
    Dim parameter5 As New SqlParameter("@ExpireDate", SqlDbType.DateTime, 8)
    parameter5.Value = expireDate
    command1.Parameters.Add(parameter5)
    Dim parameter6 As New SqlParameter("@Description", SqlDbType.NVarChar, 2000)
    parameter6.Value = description
    command1.Parameters.Add(parameter6)
    Dim parameter7 As New SqlParameter("@ImageUrl", SqlDbType.NVarChar, 256)
    parameter7.Value = imageUrl
    command1.Parameters.Add(parameter7)
    Dim parameter8 As New SqlParameter("@ImageWidth", SqlDbType.NVarChar, 5)
    parameter8.Value = imageWidth
    command1.Parameters.Add(parameter8)
    Dim parameter9 As New SqlParameter("@ImageHeight", SqlDbType.NVarChar, 5)
    parameter9.Value = imageHeight
    command1.Parameters.Add(parameter9)
    connection1.Open()
    command1.ExecuteNonQuery()
    connection1.Close()
